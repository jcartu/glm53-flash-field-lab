...
def boot(label: str, *, dcp: int, spec: str, cache: str, kv: str, kda: str = 'flashkda', fairness: str = 'compute_share', share: str = '0.4', gather: str = 'auto') -> bool:
    docker_rm()
    shm = '128g' if cache in {'lmcache', 'native'} else '32g'
    ...
    args += [
        ...
        '-v', 'r25-runtime-cache:/cache', '-v', 'r25-huggingface-cache:/root/.cache/huggingface',
    ]
    if cache == 'lmcache':
...
configs = [
    ...
    dict(label='dcp4-vram-nvfp4-mtp0', dcp=4, spec='mtp0', cache='vram', kv='nvfp4_ds_mla'),
...
for cfg in configs:
    ...
    if label == 'dcp4-lmcache-nvfp4-mtp0':
        run(['python3', PREFIX, str(PORT), 'r25-lmcache', str(ROOT / 'prefix-lmcache-80k.json'), '--tokens', '80000'], log=ROOT / 'prefix-lmcache-80k.log', timeout=1200, env={**os.environ, 'TEST_CONTAINER': NAME, 'TEST_MODEL': 'GLM-5.3-Flash-NVFP4'})
    ...
    if label == 'dcp4-native-nvfp4-mtp0':
        run(['python3', PREFIX, str(PORT), 'r25-native', str(ROOT / 'prefix-native-80k.json'), '--tokens', '80000', '--skip-restart'], log=ROOT / 'prefix-native-80k.log', timeout=900, env={**os.environ, 'TEST_CONTAINER': NAME, 'TEST_MODEL': 'GLM-5.3-Flash-NVFP4'})
