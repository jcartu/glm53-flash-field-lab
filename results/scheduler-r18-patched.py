...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # bench-session starvation guard: cap in-progress prefill chunks when
        # decode requests are runnable, so decode tokens share the step.
        # 0 disables (stock behavior).
        import os as _os
        self._starvation_guard_chunk = int(_os.environ.get("VLLM_STARVATION_GUARD_CHUNK", "0"))
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # bench-session starvation guard
            if (
                self._starvation_guard_chunk
                and request.is_prefill_chunk
                and num_new_tokens > self._starvation_guard_chunk
            ):
                _rid = request.request_id
                _decode_runnable = any(
                    not r.is_prefill_chunk
                    and r.request_id != _rid
                    and (
                ...
                if _decode_runnable:
                    num_new_tokens = self._starvation_guard_chunk
